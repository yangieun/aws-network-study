...
### [Phase 1(IKE Phase 1)]
* 서로를 인증하고, 주고 받을 협상 내용 자체를 암호화할 통로를 만드는 단계
...
### [Big-O 표기법]
* 알고리즘의 성능을 수학적으로 표시한다.
* 데이터 증가에 따른 알고리즘의 성능을 예측하는 것이 목표이다.
    * 컴퓨터에 따라서 측정된 '초'가 다를 수 있기 때문이다.
    * 따라서 상수는 제거한다. (ex. 2n -> n)
    * 또한 최고차 항만 표기한다. (ex. n^2 + n -> n^2)
* O(1)
    * constant time
    * (ex. 변수 선언)
    * 데이터가 증가함에 따라 소요시간이 변하지 않는다.
* O(n)
    * linear time
    * (ex. for loop)
    * 데이터의 증가에 비례하여 소요시간이 늘어난다.
* O(n^2)
    * quadratic time
    * (ex. 2중 for loop)
    * 제곱에 비례하여 소요시간이 늘어난다.
* O(nm)
    * quadratic time
    * (ex. 서로다른 길이의 2중 for loop)
    * 두개의 값의 곱에 비례하여 소요시간이 늘어난다.
* O(n^3)
    * cubic time
    * (ex. 3중 for loop)
    * 세제곱에 비례하여 소요시간이 늘어난다.
* O(2^n)
    * exponential time
    * (ex. 피보나치 수열 재귀함수)
    * 2에 지수로 소요시간이 늘어난다.
    * quadratic과 다르기 때문에 주의한다.
* O(log n)
    * logarithmic time
    * (ex. binary search)
    * 소요시간이 진행에 따라 절반씩 줄어든다.
* O(n!)
    * factorial time
    * 소요시간이 진행에 따라 팩토리얼로 늘어난다.
<br><br>
...
### [객체지향 요소]
* Encapsulation : 은닉, 변수와 함수를 하나의 단위로 묶는 것. 클래스에 의해 구현되는 행위로, 해당 클래스의 인스턴스 생성을 통해 클래스에 포함된 모든 것에 쉽게 접근할 수 있다. init 메서드와 self를 사용하는 것이 인캡슐레이션으로 볼 수 있다.
* Abstraction : 추상, 객체들의 공통적인 특징을 도출하는 것. 클래스를 정의하는 것 자체가 앱스트랙션이다.
* Inheritance : 상속, 자식 클래스가 부모 클래스의 특성을 그대로 물려받는 것. 기능의 일부만 변경하는 경우 ‘overriding’이라고 한다.
* Polymorphism : 다형성, 하나의 변수or함수가 상황에 따라 다른 의미로 해석될 수 있는 것.
